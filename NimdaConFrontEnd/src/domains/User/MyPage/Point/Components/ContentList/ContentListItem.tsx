...
const ContentListItem: React.FC<ContentListItemProps> = ({
  ...
  return (
    <div
      className={`w-full min-h-[80px] transition-colors ${
    ...
    >
      <div
        className="w-full min-h-[80px] py-[15px] px-[30px] flex items-center gap-4"
        style={{ boxSizing: 'border-box', paddingInline: '24px' }}
      >
        {/* 1. 좌측: 체크박스 또는 화살표 */}
        {mode === 'checkbox' ? (
          <div
            className="flex-shrink-0 flex items-center justify-center w-[30px] h-[28px] mr-1"
            style={{ flexShrink: 0 }}
            onClick={(e) => {
              e.stopPropagation();
              onToggle();
        ...
        {item.thumbnailUrl && (
          <div
            className="flex-shrink-0 w-[24px] h-[24px] rounded overflow-hidden"
            style={{ flexShrink: 0 }}
          >
            <img
        ...
        {item.authorNickname && (
          <div
            className="flex-shrink-0 flex items-center gap-[8px]"
            style={{ flexShrink: 0 }}
          >
