...
- **대회:** 문제·스코어보드 화면과 명확한 이용 불가 안내(실시간 채점 API는 단계적 출시 전)
- **커뮤니티:** 카테고리형 게시판, 리치 텍스트, 댓글, 좋아요, 사진첩, 첨부파일
- **회원:** 회원가입과 승인, 프로필, 출석, 알림, 마일리지, 프로필 장식
- **관리자:** 회원·권한, 카테고리·태그·게시글, 대회, 마일리지 관리
- **운영:** Pull Request 검증 게이트, 불변 이미지, Docker Compose, Nginx, 검증 기반 Blue/Green 전환
...
- 로그인은 한 DB 스냅샷에서 비밀번호·승인 상태·권한·`authVersion`을 함께 확인합니다. 로그아웃은 원자적으로 버전을 증가시키고, 비밀번호·승인·권한 변경은 사용자 행을 잠근 뒤 버전을 바꿉니다. 모든 인증 요청은 현재 승인 상태와 버전을 다시 확인합니다.
- 비밀번호 찾기 시작·메일 요청의 외부 응답은 가입 여부와 관계없이 같습니다. HTTP 요청은 실제 메일 발송을 기다리지 않고 동일한 비동기 작업 형태만 넘기며, 인증 코드 확인도 계정 조회와 코드 조회를 모두 수행해 시간 차이를 줄입니다. 메일 코드는 무작위 복구 요청 식별자에 묶어 원자적으로 한 번만 소비하고, 사용자 행에는 가장 최근 인증 요청 하나만 저장합니다. 비밀번호 변경 성공 시 요청을 지우고 과거 로그인 세션도 끊습니다.
- 좋아요한 글과 NC 내역은 본인 또는 관리자만 볼 수 있습니다. 게시글 응답에는 작성자 이메일과 로그인 아이디를 넣지 않습니다. V29는 기존·신규 회원의 이메일을 기본 비공개로 바꾸며, 일반 프로필 화면에서 복구 식별자인 학번을 수정할 수 없습니다.
- 관리자 카테고리와 Actuator 규칙이 공개 규칙보다 먼저 적용됩니다. 법적 안내는 V30이 정한 네 개의 불변 slug 경로만 공개하며 숫자 게시글 주소는 보호합니다. 수정·삭제 서비스는 DB의 원본 법적 식별자를 다시 확인하고 현재 관리자만 변경할 수 있게 하며, 식별자 열은 일반 게시글 수정으로 갱신되지 않습니다.
...
- 글쓰기의 이동·페이지 종료·업로드·명시적 삭제·등록은 하나의 소유권 규칙으로 동기화하여, 업로드 또는 저장 중인 파일을 정리 작업이 지우거나 다음 글에 붙이지 못합니다. 이미지 안전 변환 실패는 원본 업로드로 우회하지 않고, 로컬 업로드 전환은 서버의 명시적 `UPLOAD_STORAGE_UNAVAILABLE` 코드에만 반응합니다. V28/V30 삭제 작업표는 대소문자를 구분하는 512자 이하 파일 키를 하나만 허용하고, 표현할 수 없는 키는 메타데이터 삭제도 함께 중단합니다. 격리가 항상 우선하며 참조 중인 파일은 삭제하지 않고, 작업 잠금·같은 트랜잭션의 실패 backoff·끝까지 진행하는 ID 탐색으로 중복 실행과 영구 실패 정체를 막습니다.
- Pull Request에서는 프론트엔드·랜딩·의존성·백엔드를 모두 검증합니다. 이미지 발행과 운영 배포는 `Nimda-Security/Nimda`의 `main` push가 성공한 경우에만 실행합니다.
...
2026-07-11에 다음을 직접 확인했습니다.

- 커뮤니티 프론트엔드 lint와 production build 통과, production 의존성 취약점 0건
- 랜딩 페이지 lint와 정적 production build 통과, production 의존성 취약점 0건
- 백엔드 `mvnw.cmd -B clean verify` **116 tests** 통과, 실패·오류·skip 0
- 인증 버전·승인 상태, 단일 스냅샷 로그인, 복구 요청별 일회용 코드, exact-origin 필터, 정확한 공개 법적 문서 경로와 삭제·숨김 방지, 개인 활동, 공개 작성자 DTO, 관리자 matcher, 첨부 인증, S3 사용자·용도·크기, 이미지 픽셀 제한, 사전 할당 canonical 업로드 key, 독립 트랜잭션 롤백 정리, 삭제 격리 우선권·참조 보호와 카테고리 계층 전이를 자동 회귀 테스트로 확인
- 로컬 390px production preview에서 홈·회원가입·글쓰기·404 화면 가로 초과 0px, 보호된 글쓰기 주소 → 로그인 → 원래 글쓰기 주소 복귀, 작성 내용이 있을 때 로고 이동 취소 후 내용 유지 확인
- 운영 읽기 전용 스모크에서 데스크톱 로그인·홈 이동 성공, 미인증 관리자 카테고리 401, 신뢰하지 않는 origin preflight 403 확인
- GPT-5.6 Sol의 Pro 추론으로 첨부 트랜잭션·S3 응답 유실 경로와 법적 문서 변경·삭제 보호를 각각 다시 검토했으며 두 리뷰 모두 `RELEASE: APPROVE`를 받음. pack과 응답은 무시된 로컬 감사 자료로만 보관
...
`.gjc/`, `.insane-review/`, `audit-assets/`, `load-tests/results/`, `dist/`, `.next/`, `out/`, `target/`, 로그와 실제 환경 파일을 커밋하지 않습니다. Bruno 요청은 literal bearer token 대신 `{{jwtToken}}`을 참조합니다. 커밋 전 status, `git diff --check`, 변경 파일과 비밀 패턴을 확인합니다. GPT-5.6 Pro 웹 리뷰의 pack과 응답도 로컬 감사 자료이며 Git 변경에는 포함하지 않습니다.
