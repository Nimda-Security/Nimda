 ...
 * 결과는 requestId로만 저장되는데, 사용자는 문제 페이지에서 requestId를 들고 있지 않기 때문이다.
 *
 * <pre>
 *   ctf:instance:user-map:{userId}:{challengeId}  ─(String)─►  requestId
 *   ctf:instance:result:{requestId}               ─(Hash)───►  status/host/port/...
 * </pre>
 */
@Service
public class InstanceResultStore extends StreamResultStore<InstanceResultMessage> {

    /** 결과 키(ctf:instance:result:{requestId})와 섞이지 않도록 접두사를 따로 판다. */
    ...
    /**
     * 이 사용자가 이 문제로 요청해 둔 인스턴스의 requestId. 결과가 아직 안 왔어도(생성 중) 값이 있다.
     */
    public Optional<String> findRequestId(Long userId, Long challengeId) {
        if (userId == null || challengeId == null) {
            return Optional.empty();
        }
        String requestId = redisTemplate.opsForValue().get(userChallengeKey(userId, challengeId));
     ...
     */
    public Optional<InstanceResultMessage> findByUserIdAndChallengeId(Long userId, Long challengeId) {
        return findRequestId(userId, challengeId).flatMap(this::find);
    }

    /** userId:challengeId → requestId 매핑을 기록한다. 같은 문제로 다시 요청하면 덮어쓴다. */
    public void saveUserChallengeMapping(Long userId, Long challengeId, String requestId, Duration ttl) {
        redisTemplate.opsForValue().set(userChallengeKey(userId, challengeId), requestId, ttl);
    }
